// (c) Matt Garmon - 2017
// lzw2.cpp - Implementation of LZW compression with variable code lengths (9-16 bits)
...
void write_binary_string_file(const std::string &bits, std::string filename){
	std::ofstream output(filename+".lzw2", std::ios::binary);
	if(output){
...
std::string read_binary_string_file(std::string filename){
	std::ifstream input(filename+".lzw2", std::ios::binary|std::ios::ate);
	if(input){
		std::streampos size;
		std::string bits;
		size = input.tellg()*8;
...
void string_to_new_file(const std::string &string, std::string filename){
	...
	std::ofstream file(name + "2M" + type, std::ios::binary);
	if(file){
...
//returns code bit length based on dictionary size
//asserts if dictSize >= (1<<16)
int get_num_bits(int dictSize){
	for(int i = 9; i < 17; ++i){
    	if(dictSize < (1<<i)) return i;
  	}
  	assert(dictSize < (1<<16));
}
...
std::string compress(const std::string &uncompressed) {
	...
	int bit_count = 9; //initial code length
	...
	for (char c : uncompressed) {
		...
		if (dictionary.count(wc)) w = wc; //string exists, get next char
			...
			//update code length, and add binary code of w to output
			bit_count = get_num_bits(dictSize);
			bits += int_to_binary(dictionary[w], bit_count);

			//if more codes exist, add wc to dictionary
			if(dictSize+1 < (1<<16)) dictionary[wc] = dictSize++;
	...
	if (!w.empty()) bits += int_to_binary(dictionary[w], bit_count);
...
std::string decompress(const std::string &compressed) {
	...
	std::string bits = compressed.substr(0, bit_count);
	std::string w(1, binary_to_int(bits)); 
	std::string result = w;

	std::string entry; //to hold current output entry

	for (int i = 9; i < compressed.size(); i += bit_count) {
		//stay one step ahead of code lengths
		if(dictSize+1 < (1<<16)) bit_count = get_num_bits(dictSize+1);

		bits = compressed.substr(i, bit_count); //get next code
		if (bits.size() < bit_count) break; //handle zero buffer
		...
		if(dictSize+1 < (1<<16)) dictionary[dictSize++] = w + entry[0];
...
int main(int argc, char* argv[]){
	...
	if(std::string(argv[1]) == "c"){ //compress
		...
		std::cout << "Compressed file created: " << user_input << ".lzw2\n";
		...
		std::cout << "Decompressed file created: " << name << "2M" << type <<"\n";
	}
